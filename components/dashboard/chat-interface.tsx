...
export function ChatInterface({ documents }: { documents: Document[] }) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden relative">
      ...
      <div ref={scrollRef} className="flex-1 p-6 overflow-y-auto scroll-smooth min-h-0">
          ...
          {messages.length === 0 ? (
            <Card className="border-none shadow-none bg-transparent">
              <CardHeader className="text-center pb-2">
                ...
                <CardTitle className="text-2xl font-bold">How can I help you today?</CardTitle>
                <CardDescription>Type @ to mention a specific document for context.</CardDescription>
              </CardHeader>
            </Card>
          ) : (
            messages.map((m) => (
              <div key={m.id} className={`flex gap-4 ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                ...
                <div className={`rounded-lg px-4 py-3 max-w-[85%] text-sm leading-relaxed ${m.role === "user" ? "bg-blue-600 text-white" : "bg-white border shadow-sm dark:bg-slate-800 dark:border-slate-700"}`}>
                  {m.parts.map((p, i) => p.type === 'text' && <p key={i} className="whitespace-pre-wrap">{p.text}</p>)}
                </div>
                {m.role === "user" && (
      ...
      <div className="p-4 bg-white border-t dark:bg-slate-900 shrink-0">
        <form onSubmit={handleFormSubmit} className="max-w-3xl mx-auto relative flex flex-col">
          ...
          {selectedFile && (
            <div className="flex items-center gap-2 mb-2 self-start bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 px-3 py-1 rounded-full text-xs font-medium border border-blue-100 dark:border-blue-800 animate-in zoom-in-95">
              ...
              <button type="button" onClick={() => setSelectedFile(null)} className="hover:bg-blue-200 dark:hover:bg-blue-800 rounded-full p-0.5 transition-colors">
                <X className="h-3 w-3" />
              </button>
            </div>
          )}
          ...
          <div className="relative flex items-center">
            <Popover open={showMentions} onOpenChange={setShowMentions}>
              ...
              <PopoverContent className="p-0 w-[300px]" align="start" side="top" sideOffset={10}>
                <Command>
                  <CommandList>
                    <CommandGroup heading="Mention Document">
                      {documents.length === 0 && <div className="p-2 text-xs text-center text-muted-foreground">No documents available</div>}
                      {documents.map((doc) => (
                        <CommandItem key={doc.id} onSelect={() => handleSelectFile(doc)} className="cursor-pointer">
            ...
            <Button type="submit" size="icon" disabled={isLoading || !input.trim()} className="absolute right-1.5 h-9 w-9 rounded-md">
